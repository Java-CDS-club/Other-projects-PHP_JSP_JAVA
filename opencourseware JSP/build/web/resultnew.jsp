...
String qlist=(String)session.getAttribute("qlist");
      ...
      if(qlist.length()<=1)
    {
      out.println("<font color=red size=10>sorry insufficient question bank<br>Result not generated</font>");
    }
...
for(int z=1;z<=total;z++)
    {
 myans[z]=request.getParameter(qid[z]);
if(myans[z]==null)
    {
    myans[z]="<font color=red>unattempted</font>";
    }
    }
//ans1=request.getParameter(qid[1]);
//ans2=request.getParameter(qid[2]);
//ans3=request.getParameter(qid[3]);
...
for(int z=1;z<=total;z++)
    {
        ...
        rst=stm.executeQuery(" SELECT * FROM test where tid="+qid[z]+" ");

           while(rst.next())
               {
               String dquestion=rst.getString("question");
               String dtestans=rst.getString("testans");
                score=myans[z].equals(dtestans)?score+1:score;
             out.print("<table border='0'width='1226' bgcolor='#FFFFCC'>");
             out.print("<tr bgcolor='#9966FF'><td><font color=white size=3><strong>"+dquestion+"</strong></font></td></tr>");
             out.print(" <tr><td>you ans was :"+myans[z]+"</td></tr>");
