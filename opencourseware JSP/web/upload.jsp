...
<center>
    ...
    <tr>
      <td height="235">
         <center> <br>CREATE NEW OR SELECT FROM EXISTING LIST<br><form id="form1" action="uploadpre.jsp" method="get" >
...
Class.forName("com.mysql.jdbc.Driver");
    ...
    if(stream!=null)
        {
       sql="select distinct stream,semester from course where approved='yes' and stream='"+stream+"' order by stream asc";
        }
    else
     {
    sql="select distinct stream,semester from course where approved='yes' order by stream asc";
    }
       ResultSet rst=stm.executeQuery(sql);
          ...
          while(rst.next())
            ...
            out.println("<div id=subject><a href='uploadpre.jsp?stream="+rst.getString("stream")+"&subject="+rst.getString("semester")+"'>"+rst.getString("semester")+"</a></div>");
            out.println("</td>");
             if(counttr%4==0)
                  {
                  out.println("</tr>");
